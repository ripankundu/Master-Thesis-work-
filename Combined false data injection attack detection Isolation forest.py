...
#######Starting the false data injection attack ( Additive attack) in low voltage in the dataset#######
...
#Calculating the mean and standand deviation from the dataset for false data injection
mu2, sigma2 = 0.282876, 0.0397#set the mean and variance for the false data injection attack

attack_vector2 = np.random.normal(mu, sigma, [17,2]) #Random attack vector geneartion from the mean and standard deviation 

#injection of false data for the specific colum for the specific position of the dataset
attack_est2=df_new.loc[57910:57926, ['Bus21','Bus22']]

signal2 = attack_est2 - attack_vector2


#Adding the false data to the specific colum for the specific position of the dataset 

df_new.loc[57910:57926, ['Bus21','Bus22']] = signal2

all_attack_deu=df_new
...
all_attack_deu=pd.concat([all_attack_deu,df_time], axis = 1)
cols = all_attack_deu.columns.tolist()
cols = cols[-1:] + cols[:-1]
all_attack_deu= all_attack_deu[cols]
#all_attack2.to_csv("output_filename.csv", index=False)


plot_attack_data=all_attack_deu.loc[42010:60015, ['Time','Bus21','Bus22']]

reduced_plot_attack_data=all_attack_deu.loc[57000:60015, ['Time','Bus21','Bus22']]

all_attack_combined=all_attack_deu.loc[:,['Bus21','Bus22']]
...
all_attack_combined=pd.concat([all_attack_combined,df_label], axis = 1)
cols = all_attack_combined.columns.tolist()
cols = cols[-1:] + cols[:-1]
all_attack_combined= all_attack_combined[cols]
# all_attack_combined.to_csv("output_filename.csv", index=False)
all_attack_combined.rename( columns={0 :'labels'}, inplace=True )
all_attack_combined.labels.value_counts()

to_modify=all_attack_combined.loc[:,['labels']]
...
#Set the attack label 
all_attack_combined.iloc[[6011,6012,6013,6014,6015,6016,6017,6018,6019,6020,6021,6022,6023,6024,6025,6026,
                          57910,57911,57912,57913,57914,57915,57916,57917,57918,57919,57920,57921,57922,57923,57924,57925,57926,58013,58014,58015,58016,58017,58018,58019,58020,58021,58022,58023,58024,58025,58026,59910,59911,59912,59913,59914,59915,59916,59917,59918,59919,59920,59921,59922,59923,59924,59925,59926], 0] = 1
...
y_test= all_attack_combined.loc[:,['labels']]

data=all_attack_combined.drop('labels', axis=1)#

all_attack_combined.drop('labels', axis=1)#

train_data=all_attack_combined.drop('labels', axis=1)#
